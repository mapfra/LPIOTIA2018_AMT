...
class rFastTemplate {
	...
	var $REGEX_DYNBEG = '/(<!--\s*BEGIN\s+DYNAMIC\s+BLOCK:\s*([A-Za-z][-_A-Za-z0-9.]+)(\s*|\s+.*?)-->)/s';
	...
	// JMB pour Netspace : {  ou %7b et } ou %7d
	var $REGEX_VAR = '/(\{|%7b|%7B)[A-Za-z0-9][+-_A-Za-z0-9\']*(\}|%7d|%7D)/';
	...
	function logwrite ($msg) {
		// JMB Hack (pour eviter espace disque du dossier /tmp insuffisant qui plante le systeme)
		return;
		if ($this->DEBUGFD < 0) {
			...
			foreach (array_keys($this->TEMPLATE[$tag]['part']) as $p) {
				...
				if (strcmp ($p, 'parent') == 0) {
					...
					while (list($key,$val) = each ($this->VAR)) {
						...
						// JMB pour Netspace : {  ou %7b et } ou %7d
						//
						$tmp = str_replace ('%7b'.$key.'%7d', $val, $tmp);
						//$tmp = str_replace (rawurlencode('{'.$key.'}'), $val, $tmp);
						//echo rawurlencode('{'.$key.'}') . "<br>";
					}
					reset ($this->HANDLE);
