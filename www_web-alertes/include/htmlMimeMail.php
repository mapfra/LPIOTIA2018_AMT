...
/**
* Modifier pour NETSPACE par Jean-Michel Bruneau
* 10/05/2012 : htmlMimeMail( $encoding = 'utf-8')
*/
...
*/
	...
	function htmlMimeMail( $encoding = 'utf-8')
	{
		/**
  	...
  	*/
		...
		$this->build_params['html_charset'] = $encoding;
		$this->build_params['text_charset'] = $encoding;
		$this->build_params['head_charset'] = $encoding;
...
*/
	function setTextEncoding($encoding = 'quoted-printable')
	{
...
/**
* Accessor to add a Reply-To: header
*/
...
*/
	function setCc($cc) // JMB for Netspace
	{
		if ( ! empty( $cc)) {
...
*/
	function setBcc($bcc) // JMB for Netspace
	{
			if ( ! empty( $bcc)) {
...
*/
	function &_addTextPart(&$obj, $text)
	{
		...
		if (is_object($obj)) {
			...
			$mime_part = new Mail_mimePart($text, $params);
			return $mime_part;
		}
	}
...
*/
	function &_addHtmlPart(&$obj)
	{
		...
		if (is_object($obj)) {
			...
			$mime_part = new Mail_mimePart($this->html, $params);
			return $mime_part;
		}
	}
...
*/
	function &_addMixedPart()
	{
		...
		$mime_part = new Mail_mimePart('', $params);
		return $mime_part;
	}
...
*/
	function buildMessage($params = array())
	{
		...
		if ( isset($message)) {
			...
			$message_id = sprintf('<%s.%s@%s>', base_convert(time(), 10, 36), base_convert(rand(), 10, 36), ! empty( $_SERVER['HOST']) ? $_SERVER['HOST'] : $_SERVER['SERVER_NAME']);
...
*/
	function _encodeHeader($input, $charset = 'utf-8')
	{
		...
		if ( preg_match_all('/(\w*[\x80-\xFF]+\w*)/', $input, $matches)) {
			// Set internal character encoding to charset
			mb_internal_encoding( $charset);
			return mb_encode_mimeheader( $input, $charset, 'Q');
		} else
			return$input;
	}
	...
	function send($recipients, $type = 'mail')
	{
		...
		switch ($type) {
			case 'mail':
				...
				// JMB for Netspace
				$to = $this->_encodeHeader( ( is_array( $recipients)) ? implode( ', ', $recipients) : $recipients, $this->build_params['head_charset']);
			...
			case 'smtp':
				...
				$addresses = Mail_RFC822::parseAddressList( ( is_array( $recipients)) ? implode( ', ', $recipients) : $recipients, $this->smtp_params['helo'], null, false);
				...
				$headers[] = 'To: ' . $this->_encodeHeader( ( is_array( $recipients)) ? implode( ', ', $recipients) : $recipients, $this->build_params['head_charset']);
